...
Cash is stored on `userinfofrompublisher.fdCash`. TR/game money is stored on `userinfogame.fdGameMoney`, joined through `userinfo.fdUserNum` and `userinfo.fdUID` matching the publisher username. The in-game nickname is `userinfo.fdNickname`; keep it separate from the immutable publisher username.
